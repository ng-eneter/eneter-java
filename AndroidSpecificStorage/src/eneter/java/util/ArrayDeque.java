...
public class ArrayDeque<E> extends AbstractCollection<E> implements 
    ...
    private void checkAndExpand() {
        ...
        System.arraycopy(elements, front, newElements, 0, length
                - front);
        System.arraycopy(elements, 0, newElements, length - front,
                front);
        front = 0;
        rear = length;
     ...
     *
     */
    private void removeInternal(final int current,
            final boolean frontShift) {
        int cursor = current;
        if (frontShift) {
            while (cursor != front) {
    ...
    public int size() {
        ...
        return (front <= rear) ? (rear - front) : (rear
                + elements.length - front);
    }
